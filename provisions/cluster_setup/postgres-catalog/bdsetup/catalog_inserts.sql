...
select setval('catalog.catalog.islands_iid_seq'::regclass, 3);
...
insert into catalog.databases values(2,1,'welllogs1','postgres','test');
insert into catalog.databases values(3,2,'welllogs2','postgres','test');
...
insert into catalog.shims values (5, 0, 3, 'N/A');
select setval('catalog.catalog.shims_shim_id_seq'::regclass, 5);
...
insert into catalog.objects values(1,'region','r_regionkey,r_name,r_comment',4,4);
insert into catalog.objects values(2,'nation','n_nationkey,n_name,n_regionkey,n_comment',4,5);
insert into catalog.objects values(3,'part','p_partkey,p_name,p_mfgr,p_brand,p_type,p_size,p_container,p_retailprice,p_comment',4,4);
insert into catalog.objects values(4,'supplier','s_suppkey,s_name,s_address,s_nationkey,s_phone,s_acctbal,s_comment',4,5);
insert into catalog.objects values(5,'partsupp','ps_partkey,ps_suppkey,ps_availqty,ps_supplycost,ps_comment',4,4);
insert into catalog.objects values(6,'customer','c_custkey,c_name,c_address,c_nationkey,c_phone,c_acctbal,c_mktsegment,c_comment',4,5);
insert into catalog.objects values(7,'orders','o_orderkey,o_custkey,o_orderstatus,o_totalprice,o_orderdate,o_orderpriority,o_clerk,o_shippriority,o_comment',4,4);
insert into catalog.objects values(8,'lineitem','l_orderkey,l_partkey,l_suppkey,l_linenumber,l_quantity,l_extendedprice,l_discount,l_tax,l_returnflag,l_linestatus,l_shipdate,l_commitdate,l_receiptdate,l_shipinstruct,l_shipmode,l_comment',4,5);

insert into catalog.objects values(9,'myarray','dim1, dim2',2,6);
insert into catalog.objects values(10,'test_array','i,j,val',2,6);
insert into catalog.objects values(14,'myarray1','i, dim1, dim2',2,6);
insert into catalog.objects values (11, 'well_logs', '', 7, 7);
insert into catalog.objects values (12, 'drill_logs', '', 7, 7);
insert into catalog.objects values(13, 'welldata.productiondata', 'dateprd,well_bore_code,npd_well_bore_code,npd_well_bore_name,npd_field_code,npd_field_name,npd_facility_code,on_stream_hrs,avg_downhole_pressure,avg_downhole_temperature,avg_dp_tubing,avg_annulus_press,avg_choke_size_p,avg_whp_p,avg_wht_p,dp_choke_size,bore_oil_vol,bore_gas_vol,bore_wat_vol,bore_wi_vol,flow_kind,well_type', 2, 2);
insert into catalog.objects values(14, 'welldata.welllogsdata', 'depth,hook_load_avg,wob_avg,rpm_surface_avg,torqueabs_avg,spp_avg,rop_inst,well_bore_code', 2, 2);
select setval('catalog.objects_oid_seq'::regclass, 13);
